...
class CventClient extends SoapClient {
	...
	public function Login($acct, $username, $password) {
		$this->client = new SoapDebugClient("https://api.cvent.com/soap/V200611.ASMX?WSDL", array('trace' => true, 'exceptions' => true,'features' =>
SOAP_SINGLE_ELEMENT_ARRAYS));
	...
	public function GetRegistrations($eventId) {
			...
			if(isset($response->SearchResult->Id)) return $response;
			return false;
		}
	...
	public function SearchContactById($remaxId) {
		$response = $this->SearchByFilter('Contact', 'AndSearch', array((object) array('Field' => 'ContactId', 'Operator' => 'Equals', 'Value' => $remaxId)));
		if(isset($response->SearchResult->Id)) return $response->SearchResult->Id;
		return false;
	}
	...
	public function RetrieveRegistration($ids) {
		return $this->RetrieveAllPages('Registration', $ids);
	}

	public function RetrieveSurvey($ids) {
		return $this->RetrieveAllPages('SurveyQuestion', $ids);
	}

//QuestionId

public function RetrieveEventQuestion($ids) {
	return $this->RetrieveAllPages('EventQuestion', $ids);
}


	public function RetrieveCustom($ids) {
		return $this->RetrieveAllPages('EntityCustomField', $ids);
	}

	public function RetrieveInvitee($ids) {
		return $this->RetrieveAllPages('Invitee', $ids);
	}
