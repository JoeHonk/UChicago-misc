...
<table border=0>
 <tr>
  <td width=250><h3>Open statistics by message</h3></td>
  <td>
<?php
if ($_POST["info"]) {
 $id = $_POST["info"];
...
<form method="POST">
<?php
print 'Enter message number: <input type=text name="info" value="'.$id.'" size=7>';
?>
...
if ($id) {
 ...
 if( !$GLOBALS["require_login"] || $_SESSION["logindetails"]['superuser'] || !$notowner ) {
  ...
  print "<tr><td>Total unique views:&nbsp;</td><td>$uniqueviews[0]</td><td>&nbsp;($openrate% open rate)</td></tr></table>";
