...
foreach( $records as $record )
{
  print_r($record);
    ...
    //$guest = mb_convert_encoding($str, "SJIS");$record["FirstName"].' '.$record["LastName"],HTML-ENTITIES);
...
print_r($guests);
print jsonRemoveUnicodeSequences($guests);
...
if ($total < $lwcmin){
  ...
  $guests=json_encode($guests,JSON_FORCE_OBJECT);

//print_r($guests);

  $guests = str_replace('\u00c3\u00a9', '&eacute;',$guests);
