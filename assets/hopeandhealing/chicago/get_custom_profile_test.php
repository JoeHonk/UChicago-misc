...
function getCustomProfile($cid)
{
    ...
    //print_r($custom_profile);
    return $custom_profile['Records']['Record'];
}

function getCause($cid)
{
    global $api;
    $cause = $api->query('SELECT * FROM ContactCustomProfileField WHERE (FieldID = 6079656 AND ContactID = '.$cid.')');
    //print_r($cause);
    return $cause['Records']['Record'];
}
