...
function checkLength(ptr, maxlength) {
    //KDEV00160694 Text Area Field: Set limit to 3200 characters but you can still type in more
...
function checkCountryZip(zipptr, ctrptr, msg) {
    ...
    if ((zip != "") && (country == "CA")) {
        ...
        for (i = 0; i < zip.length; i++) {
            ...
            err = "Use the Canadian postal code format LDL DLD. Note that L=letter, D=digit; space in the middle is optional. Please update each section that includes the postal code field, such as contact and/or billing.";
        }
    }
    else {
        formatzip = zip;
...
function correspondStateCountry(objChanged, objToChange)
{
    var strToChange = "";
    switch(objChanged[objChanged.selectedIndex].value){
        case "PR":     
        case "GU":   
        case "VI":
        case "AS":
        ...
        case "MP":
        case "FM":
            strToChange = objChanged[objChanged.selectedIndex].value;
            break;
        default:
            if ( (objChanged[objChanged.selectedIndex].value != "") && (objChanged[objChanged.selectedIndex].value != "XX") && (objChanged.name=="state") )
                {
                strToChange = "US";
                }
            else if((objChanged[objChanged.selectedIndex].value != "US"))
            {
                if((objChanged[objChanged.selectedIndex].value == "XX"))
                {
                    strToChange = "";
                }
                else 
    ...
    for(var i=0;i<objToChange.length;i++)
    {
        if(objToChange[i].value == strToChange){
...
function StateCountryNotRequired(objChanged, objToChange)
{    
    var strToChange = objToChange[objToChange.selectedIndex].value;
    var objChangedValue = objChanged[objChanged.selectedIndex].value;
    
    var strList = ",PR,GU,VI,AS,PW,MH,MP,FM,";
    if ( strList.indexOf("," + objChangedValue + ",") != -1)
    {
        if(strToChange != "")
            strToChange = objChangedValue;
    }
    else if ( (objChanged.name=="state") && (objChangedValue != "") && (strToChange != "") && (strToChange != "US") )
        strToChange = "";
    else if ( (objChanged.name == "country") && (objChangedValue != "") )
    {
        if( ((objChangedValue == "US") && (strList.indexOf("," + strToChange + ",") != -1)) || objChangedValue != "US" )
            strToChange = "";                               
    }
    for(var i=0;i<objToChange.length;i++)
    {
